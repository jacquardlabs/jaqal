...
Before evaluating anything, build an accurate picture of the current state. Spawn a subagent to:
...
### Boundaries
- Are module boundaries aligned with product concepts or with technical layers?
- Are there cross-cutting concerns that should be centralized but are instead reimplemented per module?
...
- Is there data that's outgrown its current storage approach?

## Part 3 — Recommendations

For each finding, classify as:

- **REFACTOR NOW** — Actively slowing down development or causing bugs.
- **REFACTOR BEFORE [specific upcoming work]** — Will become a problem when you build [X].
- **DESIGN DECISION NEEDED** — Two valid approaches exist. Describe both with tradeoffs.
- **DOCUMENT AND ACCEPT** — Conscious tradeoff, not an accident. Add to CLAUDE.md.
